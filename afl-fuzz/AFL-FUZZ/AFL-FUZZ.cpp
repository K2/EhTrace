...
char *dir = "C:\\Users\\files\\Documents\\GitHub\\EhTrace\\EhTrace\\x64\\Debug";
/*
...
DWORD child_pid = -1,            /* PID of the fuzzed program        */
...
static void mark_as_det_done(struct queue_entry* q) {
	...
	fn = (u8 *) alloc_printf("%s\\queue\\.state\\deterministic_done\\%s", out_dir, fn + 1);
...
static void mark_as_variable(struct queue_entry* q) {
	...
	ldest = (u8 *) alloc_printf("..\\..\\%s", fn);
	fn = (u8 *)alloc_printf("%s\\queue\\.state\\variable_behavior\\%s", out_dir, fn);
...
static void mark_as_redundant(struct queue_entry* q, u8 state) {
	...
	fn = (u8 *)alloc_printf("%s\\queue\\.state\\redundant_edges\\%s", out_dir, fn + 1);
...
static void write_bitmap(void) {
	...
	fname = (u8 *)alloc_printf("%s\\fuzz_bitmap", out_dir);
...
int scandir_sorted(char *in_dir, char ***out) {
	...
	if (in_dir[strlen(in_dir) - 1] == '\\') {
		pattern = alloc_printf("%s\\%s*", dir, in_dir);
	}
	else {
		pattern = alloc_printf("%s\\%s\\*", dir, in_dir);
	}
	...
	*out = (char **)malloc(cnt * sizeof(char *));
	...
	do {
		...
		(*out)[cnt2 - 1] = alloc_printf("%s\\%s\\%s", dir, in_dir, fd.cFileName);
...
static void read_testcases(void) {
	...
	fn = (u8 *)alloc_printf("%s\\queue", in_dir);
	if (!_access((u8 *)fn, 0)) in_dir = fn; else ck_free(fn);
	...
	for (i = 0; i < nl_cnt; i++) {
		...
		u8* fn = (u8 *)alloc_printf("%s", nl[i]);
		u8* dfn = (u8 *)alloc_printf("%s\\.state\\deterministic_done\\%s", in_dir, nl[i]);

		u8  passed_det = 0;

		ck_free(nl[i]); /* not tracked */

		st_size = FileSize((char *)fn);
...
static void load_extras_file(u8* fname, u32* min_len, u32* max_len,
	...
	while ((lptr = (u8 *)fgets((char *)buf, MAX_LINE, f))) {
		...
		*rptr = 0;
		...
		if (*lptr == '@') {
			...
			lptr++;
			if ((unsigned)atoi((const char *)lptr) > dict_level) continue;
		...
		extras = (extra_data *) ck_realloc_block(extras, (extras_cnt + 1) *
...
static void load_extras(u8* dir) {
	...
	if ((x = strchr(dir, '@'))) {
		...
		*x = 0;
		dict_level = atoi((const char *)x + 1);
	...
	do {
		u8* fn = (u8 *) alloc_printf("%s\\%s", dir, fdata.cFileName);
		s32 fd;
...
static void maybe_add_auto(u8* mem, u32 len) {
	...
	if (a_extras_cnt < MAX_AUTO_EXTRAS) {
		...
		a_extras = (extra_data *)ck_realloc_block(a_extras, (a_extras_cnt + 1) *
			sizeof(struct extra_data));
...
static void save_auto(void) {
	...
	for (i = 0; i < MIN(USE_AUTO_EXTRAS, a_extras_cnt); i++) {
		...
		u8* fn = (u8 *) alloc_printf("%s\\queue\\.state\\auto_extras\\auto_%06u", out_dir, i);
		s32 fd;
...
static void load_auto(void) {
	...
	for (i = 0; i < USE_AUTO_EXTRAS; i++) {
		...
		u8* fn = (u8 *)alloc_printf("%s\\%s\\.state\\auto_extras\\auto_%06u", dir, in_dir, i);
		s32 fd, len;
...
static void create_target_process(char** argv) {
	...
	target_cmd = (u8*)argv_to_cmd(argv);
	...
	//dr_cmd = (u8 *) alloc_printf("%s\\drrun.exe -pidfile %s -c winafl.dll %s -- %s", dynamorio_dir, pidfile, client_params, target_cmd);
	sprintf(dr_cmd, "astrace -x %s\\test_gdiplus.exe -E %s\\AWinAFL.dll", dynamorio_dir, dynamorio_dir);
...
static void write_to_testcase(void* mem, u32 len) {
	...
	if (out_file) {
		...
		unlink((const char *)out_file); /* Ignore errors. */

		fd = open((const char *)out_file, O_WRONLY | O_BINARY | O_CREAT | O_EXCL, 0600);
...
static void perform_dry_run(char** argv) {
	...
	while (q) {
		...
		fd = open((const char *)q->fname, O_RDONLY | O_BINARY);
		if (fd < 0) PFATAL("Unable to open '%s'", q->fname);

		use_mem = (char *) ck_alloc_nozero(q->len);
...
static void link_or_copy(u8* old_path, u8* new_path) {
	...
	sfd = open((const char *)old_path, O_RDONLY | O_BINARY);
	if (sfd < 0) PFATAL("Unable to open '%s'", old_path);

	dfd = open((const char *)new_path, O_WRONLY | O_BINARY | O_CREAT | O_EXCL, 0600);
...
#  define CASE_PREFIX "id_"
		...
		if (!strncmp(rsl, CASE_PREFIX, 3) &&
			...
			resuming_fuzz = 1;
			nfn = (u8 *)alloc_printf("%s\\queue\\%s", out_dir, rsl);
			...
			nfn = (u8 *) alloc_printf("%s\\queue\\id:%06u,orig:%s", out_dir, id, use_name);

#else

			nfn = (u8 *)alloc_printf("%s\\%s\\queue\\id_%06u", dir, out_dir, id);
...
static void write_crash_readme(void) {
	...
	u8* fn = (u8 *) alloc_printf("%s\\crashes\\README.txt", out_dir);
	s32 fd;
	FILE* f;
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
	...
	if (fault == crash_mode) {
		...
		fn = (u8 *) alloc_printf("%s\\queue\\id:%06u,%s", out_dir, queued_paths,
			describe_op(hnb));

#else

		fn = (u8 *)alloc_printf("%s\\queue\\id_%06u", out_dir, queued_paths);
	...
	case FAULT_HANG:
		...
		fn = (u8 *) alloc_printf("%s\\hangs\\id:%06llu,%s", out_dir,
			unique_hangs, describe_op(0));

#else

		fn = (u8 *) alloc_printf("%s\\hangs\\id_%06llu", out_dir,
			unique_hangs);
	...
	case FAULT_CRASH:
		...
		fn = (u8 *) alloc_printf("%s\\crashes\\id:%06llu,sig:%02u,%s", out_dir,
			unique_crashes, kill_signal, describe_op(0));

#else

		fn = (u8 *)alloc_printf("%s\\crashes\\id_%06llu_%02u", out_dir, unique_crashes,
			kill_signal);
	...
	fd = open((const char *)fn, O_WRONLY | O_BINARY | O_CREAT | O_EXCL, 0600);
	if (fd < 0) PFATAL("Unable to create '%s'", fn);
	ck_write(fd, mem, len, fn);
	_close(fd);
...
static u32 find_start_position(void) {
	...
	if (in_place_resume) fn = (u8 *) alloc_printf("%s\\fuzzer_stats", out_dir);
	else fn = (u8 *) alloc_printf("%s\\..\\fuzzer_stats", in_dir);
...
static void find_timeout(void) {
	...
	off = (u8*)strstr(tmp, "exec_timeout   : ");
	if (!off) return;
...
static void write_stats_file(double bitmap_cvg, double eps) {
	...
	char* fn = alloc_printf("%s\\fuzzer_stats", out_dir);
	s32 fd;
	FILE* f;
...
static u8 delete_files(u8* path, u8* prefix) {
	...
	do {
		if (fd.cFileName[0] != '.' && (!prefix ||
			...
			u8* fname = (u8 *) alloc_printf("%s\\%s", path, fd.cFileName);
...
static void nuke_resume_dir(void) {
	...
	fn = (u8 *) alloc_printf("%s\\_resume\\.state\\deterministic_done", out_dir);
	if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
	ck_free(fn);

	fn = (u8 *) alloc_printf("%s\\_resume\\.state\\auto_extras", out_dir);
	if (delete_files(fn, "auto_")) goto dir_cleanup_failed;
	ck_free(fn);

	fn = (u8 *) alloc_printf("%s\\_resume\\.state\\redundant_edges", out_dir);
	if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
	ck_free(fn);

	fn = (u8 *) alloc_printf("%s\\_resume\\.state\\variable_behavior", out_dir);
	if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
	ck_free(fn);

	fn = (u8 *) alloc_printf("%s\\_resume\\.state", out_dir);
	if (rmdir(fn) && errno != ENOENT) goto dir_cleanup_failed;
	ck_free(fn);

	fn = (u8 *) alloc_printf("%s\\_resume", out_dir);
	if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
	ck_free(fn);
...
static void maybe_delete_out_dir(void) {
	...
	FILE* f;
	u8 *fn = (u8 *) alloc_printf("%s\\fuzzer_stats", out_dir);
	...
	if (in_place_resume) {
		...
		u8* orig_q = (u8 *) alloc_printf("%s\\queue", out_dir);

		in_dir = (u8 *) alloc_printf("%s\\_resume", out_dir);
	...
	fn = (u8 *) alloc_printf("%s\\.synced", out_dir);
	if (delete_files(fn, NULL)) goto dir_cleanup_failed;
	ck_free(fn);

	/* Next, we need to clean up <out_dir>/queue/.state/ subdirectories: */

	fn = (u8 *) alloc_printf("%s\\queue\\.state\\deterministic_done", out_dir);
	if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
	ck_free(fn);

	fn = (u8 *) alloc_printf("%s\\queue\\.state\\auto_extras", out_dir);
	if (delete_files(fn, "auto_")) goto dir_cleanup_failed;
	ck_free(fn);

	fn = (u8 *) alloc_printf("%s\\queue\\.state\\redundant_edges", out_dir);
	if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
	ck_free(fn);

	fn = (u8 *) alloc_printf("%s\\queue\\.state\\variable_behavior", out_dir);
	if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
	ck_free(fn);
	...
	fn = (u8 *) alloc_printf("%s\\queue\\.state", out_dir);
	if (rmdir(fn) && errno != ENOENT) goto dir_cleanup_failed;
	ck_free(fn);

	fn = (u8 *) alloc_printf("%s\\queue", out_dir);
	if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
	ck_free(fn);
	...
	if (!in_place_resume) {
		...
		fn = (u8 *) alloc_printf("%s\\crashes\\README.txt", out_dir);
		unlink(fn); /* Ignore errors */
		ck_free(fn);

	}

	fn = (u8 *) alloc_printf("%s\\crashes", out_dir);
	...
	if (in_place_resume && rmdir(fn)) {
		...
		u8* nfn = (u8 *) alloc_printf("%s.%04u-%02u-%02u-%02u:%02u:%02u", fn,
			t->tm_year + 1900, t->tm_mon + 1, t->tm_mday,
			t->tm_hour, t->tm_min, t->tm_sec);

#else

		u8* nfn = (u8 *) alloc_printf("%s_%04u%02u%02u%02u%02u%02u", fn,
			t->tm_year + 1900, t->tm_mon + 1, t->tm_mday,
			t->tm_hour, t->tm_min, t->tm_sec);
	...
	fn = (u8 *) alloc_printf("%s\\hangs", out_dir);
	...
	fn = (u8 *) alloc_printf("%s\\.cur_input", out_dir);
	if (unlink(fn) && errno != ENOENT) goto dir_cleanup_failed;
	ck_free(fn);

	fn = (u8 *) alloc_printf("%s\\fuzz_bitmap", out_dir);
	if (unlink(fn) && errno != ENOENT) goto dir_cleanup_failed;
	ck_free(fn);

	if (!in_place_resume) {
		fn = (u8 *) alloc_printf("%s\\fuzzer_stats", out_dir);
		if (unlink(fn) && errno != ENOENT) goto dir_cleanup_failed;
		ck_free(fn);
	}

	fn = (u8 *) alloc_printf("%s\\plot_data", out_dir);
	if (unlink(fn) && errno != ENOENT) goto dir_cleanup_failed;
	ck_free(fn);
...
static u8 fuzz_one(char** argv) {
	...
	out_buf = (char *) ck_alloc_nozero(len);
...
#define EFF_SPAN_ALEN(_p, _l) (EFF_APOS((_p) + (_l) - 1) - EFF_APOS(_p) + 1)
	...
	eff_map = (char *)ck_alloc(EFF_ALEN(len));
	eff_map[0] = 1;
...
skip_interest:
	...
	********************/
	...
	ex_tmp = (char *)ck_alloc(len + MAX_DICT_FILE);
...
havoc_stage:
	...
	for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
		...
		for (i = 0; i < use_stacking; i++) {
			...
			case 13:
				...
				if (temp_len + HAVOC_BLK_LARGE < MAX_FILE) {
					...
					new_buf = (char *)ck_alloc_nozero(temp_len + clone_len);
			...
			case 17: {
				...
				if (!extras_cnt || (a_extras_cnt && UR(2))) {
					...
					new_buf = (char *)ck_alloc_nozero(temp_len + extra_len);
		...
		if (temp_len < len) out_buf = (char *)ck_realloc(out_buf, len);
		temp_len = len;
		memcpy(out_buf, in_buf, len);
...
retry_splicing:
	...
	if (use_splicing && splice_cycle++ < SPLICE_CYCLES &&
		...
		new_buf = (char *)ck_alloc_nozero(target->len);
...
static void sync_fuzzers(char** argv) {
	...
	find_pattern = (u8 *) alloc_printf("%s\\*", sync_dir);
	...
	do {
		...
		qd_path = (u8 *) alloc_printf("%s\\%s\\queue", sync_dir, sd.cFileName);
		qd_path_pattern = (u8 *) alloc_printf("%s\\*", qd_path);
		...
		qd_synced_path = (u8 *) alloc_printf("%s\\.synced\\%s", out_dir, sd.cFileName);
		...
		do {
			...
			path = (u8 *) alloc_printf("%s\\%s", qd_path, qd.cFileName);
...
static void setup_dirs_fds(void) {
	...
	tmp = (u8 *) alloc_printf("%s\\queue", out_dir);
	...
	tmp = (u8 *) alloc_printf("%s\\queue\\.state\\", out_dir);
	if (mkdir(tmp)) PFATAL("Unable to create '%s'", tmp);
	ck_free(tmp);
	...
	tmp = (u8 *) alloc_printf("%s\\queue\\.state\\deterministic_done\\", out_dir);
	if (mkdir(tmp)) PFATAL("Unable to create '%s'", tmp);
	ck_free(tmp);

	/* Directory with the auto-selected dictionary entries. */

	tmp = (u8 *) alloc_printf("%s\\queue\\.state\\auto_extras\\", out_dir);
	if (mkdir(tmp)) PFATAL("Unable to create '%s'", tmp);
	ck_free(tmp);

	/* The set of paths currently deemed redundant. */

	tmp = (u8 *) alloc_printf("%s\\queue\\.state\\redundant_edges\\", out_dir);
	if (mkdir(tmp)) PFATAL("Unable to create '%s'", tmp);
	ck_free(tmp);

	/* The set of paths showing variable behavior. */

	tmp = (u8 *) alloc_printf("%s\\queue\\.state\\variable_behavior\\", out_dir);
	if (mkdir(tmp)) PFATAL("Unable to create '%s'", tmp);
	ck_free(tmp);
	...
	if (sync_id) {
		...
		tmp = (u8 *) alloc_printf("%s\\.synced\\", out_dir);
		if (mkdir(tmp)) PFATAL("Unable to create '%s'", tmp);
		ck_free(tmp);
	...
	tmp = (u8 *) alloc_printf("%s\\crashes", out_dir);
	if (mkdir(tmp)) PFATAL("Unable to create '%s'", tmp);
	ck_free(tmp);

	/* All recorded hangs. */

	tmp = (u8 *) alloc_printf("%s\\hangs", out_dir);
	if (mkdir(tmp)) PFATAL("Unable to create '%s'", tmp);
	ck_free(tmp);
	...
	tmp = (u8 *) alloc_printf("%s\\plot_data", out_dir);
...
static void setup_stdio_file(void) {
	...
	u8* fn = (u8 *) alloc_printf("%s\\.cur_input", out_dir);
...
static void fix_up_sync(void) {
	...
	x = (u8 *) alloc_printf("%s\\%s", out_dir, sync_id);
...
static void detect_file_args(char** argv) {
	...
	while (argv[i]) {
		...
		if (aa_loc) {
			...
			if (!out_file)
				out_file = (u8 *) alloc_printf("%s\\.cur_input", out_dir);

			/* Be sure that we're always using fully-qualified paths. */

			//if (out_file[0] == '\\') aa_subst = out_file;
			//else aa_subst = (u8 *) alloc_printf("%s\\%s", cwd, out_file);
			aa_subst = out_file;

			/* Construct a replacement argv value. */

			*aa_loc = 0;
			n_arg = (u8 *) alloc_printf("%s%s%s", argv[i], aa_subst, aa_loc + 2);
			argv[i] = n_arg;
			*aa_loc = '@';
...
static void save_cmdline(u32 argc, char** argv) {
	...
	buf = orig_cmdline = (char *)ck_alloc(len);
...
static void extract_client_params(u32 argc, char** argv) {
	...
	buf = client_params = (char *)ck_alloc(len);
